...
module OpenapiClient
  ...
  class ProcessingOptions < ApiModelBase
    ...
    attr_accessor :verify_destination_parameters
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ProcessingOptions` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ProcessingOptions`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'validate_xml')
        self.validate_xml = attributes[:'validate_xml']
      else
    ...
    def hash
      [facturx_profile, auto_enrich, validate_xml, verify_destination_parameters].hash
    end
