...
module OpenapiClient
  class AFNORUpdatePutRoutingCodeBody < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::AFNORUpdatePutRoutingCodeBody` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::AFNORUpdatePutRoutingCodeBody`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'routing_identifier_type')
        ...
        self.routing_identifier_type = nil
      end
    ...
    def list_invalid_properties
      ...
      if @routing_identifier_type.nil?
        invalid_properties.push('invalid value for "routing_identifier_type", routing_identifier_type cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @routing_identifier_type.nil?
      return false if @routing_identifier_type.to_s.length > 4
      return false if @routing_identifier_type !~ Regexp.new(/^(?!\s*$).+/)
