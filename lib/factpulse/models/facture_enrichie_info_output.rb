...
module FactPulse
  ...
  class FactureEnrichieInfoOutput < ApiModelBase
    attr_accessor :numero_facture

    attr_accessor :id_emetteur

    attr_accessor :id_destinataire

    attr_accessor :nom_emetteur

    attr_accessor :nom_destinataire
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FactPulse::FactureEnrichieInfoOutput` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FactPulse::FactureEnrichieInfoOutput`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] nom_emetteur Value to be assigned
    def nom_emetteur=(nom_emetteur)
      if nom_emetteur.nil?
        fail ArgumentError, 'nom_emetteur cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] nom_destinataire Value to be assigned
    def nom_destinataire=(nom_destinataire)
      if nom_destinataire.nil?
        fail ArgumentError, 'nom_destinataire cannot be nil'
      end
