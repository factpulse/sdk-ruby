...
module FactPulse
  ...
  class ReportIssuer < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FactPulse::ReportIssuer` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FactPulse::ReportIssuer`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @siren.to_s.length > 14
        invalid_properties.push('invalid value for "siren", the character length must be smaller than or equal to 14.')
      end

      if @siren.to_s.length < 9
        invalid_properties.push('invalid value for "siren", the character length must be greater than or equal to 9.')
      end
    ...
    def valid?
      ...
      return false if @siren.to_s.length > 14
    ...
    def siren=(siren)
      ...
      if siren.to_s.length > 14
        fail ArgumentError, 'invalid value for "siren", the character length must be smaller than or equal to 14.'
      end

      if siren.to_s.length < 9
        fail ArgumentError, 'invalid value for "siren", the character length must be greater than or equal to 9.'
      end
    ...
    def hash
      [siren, name, vat_id, role_code].hash
    end
