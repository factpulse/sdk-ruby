...
module FactPulse
  ...
  class ValidateCDARRequest < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'status' => :'String',
        :'reason_code' => :'String',
        :'encaisse_amount' => :'Encaisseamount1'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FactPulse::ValidateCDARRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FactPulse::ValidateCDARRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [document_id, sender_siren, sender_role, invoice_id, invoice_issue_date, status, reason_code, encaisse_amount].hash
    end
