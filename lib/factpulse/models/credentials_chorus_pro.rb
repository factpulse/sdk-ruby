...
module FactPulse
  ...
  class CredentialsChorusPro < ApiModelBase
    ...
    def self.attribute_map
      {
        ...
        :'chorus_login' => :'chorus_login',
        :'chorus_password' => :'chorus_password',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FactPulse::CredentialsChorusPro` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FactPulse::CredentialsChorusPro`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'mode_sandbox')
        self.mode_sandbox = attributes[:'mode_sandbox']
      else
    ...
    def hash
      [piste_client_id, piste_client_secret, chorus_login, chorus_password, mode_sandbox].hash
    end
