...
module FactPulse
  ...
  class InvoiceLifecycle < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FactPulse::InvoiceLifecycle` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FactPulse::InvoiceLifecycle`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @total_events.nil?
        invalid_properties.push('invalid value for "total_events", total_events cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @total_events.nil?
      true
    end
    ...
    def hash
      [seller_id, invoice_id, events, total_events].hash
    end
